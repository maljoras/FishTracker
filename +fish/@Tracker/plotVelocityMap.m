function varargout = plotVelocityMap(self,plotTimeRange,fishIds,minmaxvel)
% PLOTVELOCITYMAP(SELF,PLOTTIMERANGE,FISHIDS,MAXVEL) plots a spatial
...
% of having a velocity in this range.
  ...
  velocity = [zeros(1,2,self.nfish);bsxfun(@rdivide,diff(pos,1),diff(res.tabs))];
  avelocity = squeeze(sqrt(sum(velocity.^2,2)))/self.fishlength;
  avelocity = avelocity(:,fishIds);
  ...
  if nargout
    ...
    [r1,r2] = getsubplotnumber(length(fishIds));
