...
% fish.
  ...
  methods    
    ...
    % plots the traces for a current assignemts. Called from xy.Tracker
      ...
      identityIdx = identityIds(assignments(:, 1));
      
      h = self.plotTraces(identityIdx,detectionIdx,[],stepsback);
      if nargout
        varargout{1} = h;
      end
    ...
    function varargout =checkOverlap(self,objidx,verbosity);
      ...
      for i = 1:self.nindiv
        for j = i+1:self.nindiv
          ...
          if sum(eqmsk(:,s))>1
            ...
            allL = [allL;L];
            if verbosity>1
              xy.helper.verbose('[%d,%d] Found %1.2f%% [%d] overlap. ', i,j,mean(eqmsk(:,s))*100,sum(eqmsk(:,s)))
              xy.helper.verbose('Median length %d. Longest: [ %s] \n', median(L),sprintf('%d ',sL(1:min(end,3))));
            end
          end
      ...
      if verbosity==1
        ...
        xy.helper.verbose('Found %1.2f%% [%d] overlap. ', mean(eqmsk(:))*100,sum(eqmsk(:)))
        xy.helper.verbose('Median length %d. Longest: [ %s]', median(allL),sprintf('%d ',sallL(1:min(end,8))));
