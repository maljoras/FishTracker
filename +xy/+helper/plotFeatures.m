...
if PLOTFEATURES
  ...
  for j = 1:nanimals
    for i = 1:nframes
      ...
      a = subsubplot(3,1,1,nanimals,nframes,s);
      ...
      a = subsubplot(3,1,2,nanimals,nframes,s);
      ...
      a = subsubplot(3,1,3,nanimals,nframes,s);
      img = double(seg.fishFeature(:,:,1))/nanimals;
      [~,cl] = max(track.classProb);
      if seg.bendingStdValue>bendingThres
        xy.helper.imagescbw(img,[-1,1]);
      else
        imagesc(img+(cl-1)/nanimals,[0,1]);
      end
      axis off;      
...
if PLOTCLASSMEANS
  ...
  mu = fcl.getMeans();
  for i = 1:nanimals
    subplot(2,nanimals,i)
    m = squeeze(mu(i,:,:,1));
    imagesc(m,[0.2,0.45]);
    axis off
    k = (size(C,1)-2)/nanimals*i;
    c = C(floor(k)+1,:);
  ...
  a = subplot(2,2,4);
  p = zeros(2,nframes,nanimals);
  msk = ~ones(nframes,nanimals);
  for j = 1:nanimals
    for i = 1:nframes
      s = s+1;
      track = res.tracks(iframe+(i-1)*nskip,j);
