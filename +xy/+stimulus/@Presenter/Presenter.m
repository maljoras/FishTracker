classdef Presenter < handle;
  ...
  properties 
    ...
    usePredIdentityId = false;  % wether to use the ID predicted by
  ...
  methods 
      ...
      if length(varargin)==1 && isstruct(varargin{1});
        ...
        for i = 1:2:length(varargin)
          if ~ischar(varargin{i}) || ~isprop(self,varargin{i});
            error('expect valid property name xy.stimulus.Presenter');
          end
    ...
    function identityIds = getIdentityIdsFromTracks(self,tracks)
      if self.usePredIdentityId
        identityIds = [tracks.predIdentityId];
      else
        identityIds = [tracks.identityId];
      end
    end
