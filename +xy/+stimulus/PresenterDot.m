classdef PresenterDot < xy.stimulus.Presenter;
  ...
  properties
    ...
    funStmIdentityIdProb = @(identityIds) (identityIds==1);
  ...
  methods
   ...
   function [x,y] = plotStimulus(self,x,y,t,identityIds,lrswitch)
     ...
     msk = self.stmState(identityIds) & self.stimulatedMsk;
   ...
   %       1 :  Stimulation period (+ 8 LR switch)
     ...
     if isempty(self.stimulatedMsk)
       self.stimulatedMsk = zeros(length(identityIds),1);
     end
     ...
     if self.stmIdx==self.ID_STIMULATION
       [x,y] = self.plotStimulus(x,y,t,identityIds,self.stimulatedMsk); 
     end

     % save stm info
     stmInfo = zeros(length(identityIds),5);
