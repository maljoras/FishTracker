classdef PresenterOnlineLearningCue < xy.stimulus.Presenter;
  ...
  properties
    ...
    funLRStm = @(identityIds) -(identityIds>0);
  ...
  methods
    ...
    % inits all possible backgrounds
      ...
      for i = 1:l
        ...
        texture = xy.helper.makeTexture('f2ori',max(self.windowSize),'ori',ori,...
   ...
   function [stmbbox] = plotStimulus(self,x,y,t,identityIds,lrswitch)
     ...
     if lrswitch
       stmleft = self.funLRStm(identityIds(:))>0;
       stmright = self.funLRStm(identityIds(:))<0;
     else
       stmleft = self.funLRStm(identityIds(:))<0;
       stmright = self.funLRStm(identityIds(:))>0;
     end
     ...
     stmSize = self.stmSize(min(identityIds,end));

     left = xleft & stmleft & self.stmState(identityIds);     
     right = ~xleft & stmright & self.stmState(identityIds);
     lr = left | right;
   ...
   function bboxes = transformBbox(self,bboxes,ori,identityIds)

     stmSizeFactor = self.stmSizeFactor(min(identityIds,end));
     stmShift = self.stmShift(min(identityIds,end));     
     stmShiftOri = self.stmShiftOri(min(identityIds,end));     
   ...
   %       6 :  Pause (if funRoundStmPause(iround)==0)  
     ...
     switch self.stmIdx
       ...
       otherwise
         ...
         if self.stmIdx==self.ID_STIMULUS_LR || self.stmIdx==self.ID_STIMULUS_RL
           stmbbox = self.plotStimulus(x,y,t,identityIds,lrswitch); 
         end
     ...
     % save stm info
     stmInfo = zeros(length(identityIds),8);
