...
% DAG-based tracks
  ...
  if isempty(self.res)
    ...
    if dagif 
      xy.helper.verbose('Getting DAGraph tracking results')
      res = self.res.dag;
    else
      xy.helper.verbose('Getting Switch-based tracking results')
      res = self.res.swb;
    end
  end
  ...
  try
    if self.videoHandler.resizeif
      xy.helper.verbose(['WARNING: resizeif set! Adjust ONLY "pos" ' ...
