...
% turnings from RES
  ...
  locvel = xy.helper.movavg(locvel,max(round(deltat/2),1));
  ...
  for i = 1:size(velocity,2)
    ...
    turn(i).sattv = accumarray(accmsk,vel(:,i),[mxt+1,1],@xy.helper.stderr);
    ...
    turn(i).sattlocv = accumarray(accmsk,locvel(:,i),[mxt+1,1],@xy.helper.stderr);
