function [FV,eVar,Proj,r] = lfd(Z,iidx,nfish,npca,whiteif)
% [FV,EVAR,PROJ,RSTRUC] = LFD(Z,IIDX,NFISH,NPCA) computes the LInear Fisher
...
% LFD(..,WHITEIF) whites the data after PCA (default 0). 
  ...
  Proj = bsxfun(@minus,X,mu)*r.V(:,1:nfish);
  FV = r.V(:,1:nfish);
